...
def test_example_suite_intake_detects_language_and_materializes_snapshot(tmp_path: Path):
    ...
    workspace = Workspace(tmp_path / "ws" / "VAS-0001", "VAS-0001")
    ...
    assert intake.snapshot_ref == "input_snapshot"
    metadata = json.loads(workspace.example_suite_metadata_path.read_text(encoding="utf-8"))
    assert metadata["content_digest"] == inspection.content_digest
    assert metadata["snapshot_path"] == intake.snapshot_path
