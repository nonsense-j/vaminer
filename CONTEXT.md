...
- **Anchor Intent**: host-owned behavior, inspection guidance, intent weight, and required Case Artifacts for one causal-chain hotspot, optionally accompanied by an unvalidated query draft for the Synthesizer. Its `behavior_weight` is the behavior's importance to the overall defect analysis: higher means more defect-relevant, not easier to query.
- **Anchor Plan**: an ordered, complete set of Anchor Intents plus the normative VAS summary.
- **Query weight**: the importance of matches from an implemented Anchor query, using the same scale as the target intent's `behavior_weight`. It normally equals that weight and may be lower only for an unavoidable semantic downgrade; it never exceeds it.
...
- RCA is the only Agent that writes workspace data, and it writes only through typed Case Artifact operations. Synthesizers return bounded experiences rather than writing directly; the host accepts only the final output from each Synthesizer, applies the turns-quality gate, and merges up to three deduplicated lessons into the shared ast-grep skill's common or language-specific experience file with a process-safe read/write lock and atomic replacement. Cleanup is explicit; acceptance and cache loading are pure.
