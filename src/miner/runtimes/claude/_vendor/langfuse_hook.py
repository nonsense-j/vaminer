...
Vendored from langfuse/Claude-Observability-Plugin 1.0.0 under the MIT license.
VAMiner adds an in-process entry point and configurable CLI identity so
compatible Claude CLI implementations remain distinguishable.
"""
...
CLI_NAME = Path(_opt("CC_LANGFUSE_CLI_NAME") or "claude").name or "claude"
...
# Constant per hook process so name-based grouping stays useful for each CLI.
...
def _configure_vaminer_runtime(
    ...
    global CLI_NAME, CLI_SLUG, CLI_DISPLAY_NAME, TRACE_NAME, _logger
...
def emit_vaminer_session(
    ...
    *,
    ...
    """Emit a completed VAMiner CLI session with an existing Langfuse client.

    The caller owns client flushing and shutdown. Calls are serialized because
    the upstream hook stores its cursor paths and display identity in globals.
    """
    ...
    if traceparent:
        ...
        if parent_context is None:
            raise ValueError("invalid CC_LANGFUSE_TRACEPARENT")
        parent_trace_id, parent_span_id = parent_context
        trace_seed = None
