...
from ..runtime import AgentRunResult, AgentTask, RuntimeIdentity, RuntimeRouter
...
class _WorkflowBase:
    ...
    async def _run_task(self, task: AgentTask[Any]) -> AgentRunResult[Any]:
        ...
        logger.info(
            ...
            "requests=%s turns=%s input_tokens=%s output_tokens=%s cost_usd=%s events=%s",
            task.phase.value,
            task.task_id,
            result.runtime_id,
            result.model_id,
...
class IssueWorkflow(_WorkflowBase):
    ...
    async def run(
        ...
        *,
        ...
        agent_runs: list[AgentRunResult[Any]] = []
        issue_task = make_issue_collection_task(issue_input, workspace_root=workspace.root)
        ...
        workspace.save_analysis(render_root_cause_analysis(root_cause))
